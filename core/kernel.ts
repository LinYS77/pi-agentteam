...
export type AgentTeamKernelAdapter = {
  ...
  createDetachedSwarmWindowAsync(sessionName: string, windowName: string, signal?: AbortSignal): Promise<AgentTeamKernelDetachedSwarmWindowCreation>
...
function compactTmuxPaneId(value: unknown): string {
  const raw = String(value ?? '').trim()
  if (!raw || raw.length > KERNEL_DIAGNOSTIC_TEXT_LIMIT) return ''
  return /^%\d+$/.test(raw) ? raw : ''
}

function isValidPaneLabelArgument(value: unknown): value is string {
  return typeof value === 'string' && value.length <= PANE_LABEL_ARGUMENT_LIMIT
}

function isValidOptionalTmuxOpaqueArgument(value: unknown): value is string | undefined {
  return value === undefined || (typeof value === 'string' && value.length <= TMUX_OPAQUE_ARGUMENT_LIMIT && !value.includes('\0'))
}
...
export function createAgentTeamKernelAdapter(options: AgentTeamKernelAdapterOptions = {}): AgentTeamKernelAdapter {
  ...
  function workerLifecycleUnavailableDetachedSwarmWindowCreation(sessionName: string, windowName: string, kind: AgentTeamKernelWorkerLifecycleFailureKind): AgentTeamKernelDetachedSwarmWindowCreation {
    const safeSessionName = compactTmuxSessionName(sessionName)
    const safeWindowName = compactTmuxWindowName(windowName)
    const reason = compactKernelText(`Go worker lifecycle createDetachedSwarmWindow unavailable (${kind})`, 'Go worker lifecycle createDetachedSwarmWindow unavailable (previous-helper-failure)')
    return {
      ok: false,
      operation: 'createDetachedSwarmWindow',
      capability: 'workerLifecycle',
  ...
  function workerLifecycleUnavailableWindowPaneLabelsRefresh(target: string, kind: AgentTeamKernelWorkerLifecycleFailureKind, detail?: unknown): AgentTeamKernelWindowPaneLabelsRefresh {
    const safeTarget = compactTmuxWindowTarget(target)
    const reason = kind === 'invalid-target'
      ? 'Go worker lifecycle refreshWindowPaneLabels unavailable (invalid-target)'
      : cutoverMessage(kind as AgentTeamKernelCutoverFailureKind, detail)
    const safeReason = compactKernelText(reason, cutoverMessage('previous-helper-failure'))
    return {
      ok: false,
  ...
  function workerLifecycleUnavailablePaneLabelSetting(paneId: string, kind: AgentTeamKernelWorkerLifecycleFailureKind, detail?: unknown): AgentTeamKernelPaneLabelSetting {
    const safePaneId = compactTmuxPaneId(paneId)
    const reason = kind === 'invalid-pane-id' || kind === 'invalid-label'
      ? `Go worker lifecycle setPaneLabel unavailable (${kind})`
      : cutoverMessage(kind as AgentTeamKernelCutoverFailureKind, detail)
    const safeReason = compactKernelText(reason, cutoverMessage('previous-helper-failure'))
    return {
      ok: false,
  ...
  function workerLifecycleUnavailablePaneLabelClearing(paneId: string, kind: AgentTeamKernelWorkerLifecycleFailureKind, detail?: unknown): AgentTeamKernelPaneLabelClearing {
    const safePaneId = compactTmuxPaneId(paneId)
    const reason = kind === 'invalid-pane-id'
      ? 'Go worker lifecycle clearPaneLabel unavailable (invalid-pane-id)'
      : cutoverMessage(kind as AgentTeamKernelCutoverFailureKind, detail)
    const safeReason = compactKernelText(reason, cutoverMessage('previous-helper-failure'))
    return {
      ok: false,
  ...
  function workerLifecycleUnavailableTeammatePaneCreation(target: string, paneId: string, kind: AgentTeamKernelWorkerLifecycleFailureKind): AgentTeamKernelTeammatePaneCreation {
    const safeTarget = compactTmuxWindowTarget(target)
    const safePaneId = compactTmuxPaneId(paneId)
    const reason = compactKernelText(`Go worker lifecycle createTeammatePane unavailable (${kind})`, 'Go worker lifecycle createTeammatePane unavailable (previous-helper-failure)')
    return {
      ok: false,
      operation: 'createTeammatePane',
      capability: 'workerLifecycle',
  ...
  function validateDetachedSwarmWindowCreationResult(value: unknown, requestedSessionName: string, requestedWindowName: string): AgentTeamKernelDetachedSwarmWindowCreation | undefined {
    if (!value || typeof value !== 'object' || Array.isArray(value)) return undefined
    const result = value as Partial<AgentTeamKernelDetachedSwarmWindowCreation>
    if (result.operation !== 'createDetachedSwarmWindow' || result.capability !== 'workerLifecycle') return undefined
    if (result.readOnly !== false || result.stateFilesRead !== false || result.stateFilesWritten !== false || result.tmuxMutation !== true) return undefined
    const sessionName = compactTmuxSessionName(result.sessionName || requestedSessionName)
    const windowName = compactTmuxWindowName(result.windowName || requestedWindowName)
    if (result.ok === true) {
    ...
    const failureKind = typeof result.failureKind === 'string' && ['unsupported-operation', 'invalid-session', 'invalid-window-name', ...AGENTTEAM_KERNEL_CUTOVER_FAILURE_KINDS].includes(result.failureKind)
      ? result.failureKind as AgentTeamKernelWorkerLifecycleFailureKind
      : 'previous-helper-failure'
    const reason = compactKernelText(`Go worker lifecycle createDetachedSwarmWindow unavailable (${failureKind})`, 'Go worker lifecycle createDetachedSwarmWindow unavailable (previous-helper-failure)')
    return {
      ok: false,
      operation: 'createDetachedSwarmWindow',
      capability: 'workerLifecycle',
  ...
  function validateWindowPaneLabelsRefreshResult(value: unknown, requestedTarget: string): AgentTeamKernelWindowPaneLabelsRefresh | undefined {
    if (!value || typeof value !== 'object' || Array.isArray(value)) return undefined
    const result = value as Partial<AgentTeamKernelWindowPaneLabelsRefresh>
    if (result.operation !== 'refreshWindowPaneLabels' || result.capability !== 'workerLifecycle') return undefined
    if (result.readOnly !== false || result.stateFilesRead !== false || result.stateFilesWritten !== false || result.tmuxMutation !== true) return undefined
    const target = compactTmuxWindowTarget(result.target || requestedTarget)
    if (result.ok === true) {
      if (!target || result.refreshed !== true) return undefined
      return {
        ok: true,
        operation: 'refreshWindowPaneLabels',
        capability: 'workerLifecycle',
    ...
    if (result.ok !== false || result.refreshed !== false) return undefined
    const failureKind = typeof result.failureKind === 'string' && ['unsupported-operation', 'pane-not-found', 'invalid-pane-id', 'invalid-target', 'invalid-session', ...AGENTTEAM_KERNEL_CUTOVER_FAILURE_KINDS].includes(result.failureKind)
      ? result.failureKind as AgentTeamKernelWorkerLifecycleFailureKind
      : 'previous-helper-failure'
    const reason = compactKernelText(result.reason || result.error || cutoverMessage(failureKind as AgentTeamKernelCutoverFailureKind), cutoverMessage('previous-helper-failure'))
  ...
  function validatePaneLabelSettingResult(value: unknown, requestedPaneId: string): AgentTeamKernelPaneLabelSetting | undefined {
    if (!value || typeof value !== 'object' || Array.isArray(value)) return undefined
    const result = value as Partial<AgentTeamKernelPaneLabelSetting>
    if (result.operation !== 'setPaneLabel' || result.capability !== 'workerLifecycle') return undefined
    if (result.readOnly !== false || result.stateFilesRead !== false || result.stateFilesWritten !== false || result.tmuxMutation !== true) return undefined
    const paneId = compactTmuxPaneId(result.paneId || requestedPaneId)
    if (result.ok === true) {
      if (!paneId || result.labeled !== true) return undefined
      return {
        ok: true,
        operation: 'setPaneLabel',
        capability: 'workerLifecycle',
    ...
    if (result.ok !== false || result.labeled !== false) return undefined
    const failureKind = typeof result.failureKind === 'string' && ['unsupported-operation', 'pane-not-found', 'invalid-pane-id', 'invalid-target', 'invalid-session', 'invalid-label', ...AGENTTEAM_KERNEL_CUTOVER_FAILURE_KINDS].includes(result.failureKind)
      ? result.failureKind as AgentTeamKernelWorkerLifecycleFailureKind
      : 'previous-helper-failure'
    const reason = compactKernelText(`Go worker lifecycle setPaneLabel unavailable (${failureKind})`, 'Go worker lifecycle setPaneLabel unavailable (previous-helper-failure)')
    return {
      ok: false,
      operation: 'setPaneLabel',
      capability: 'workerLifecycle',
  ...
  function validatePaneLabelClearingResult(value: unknown, requestedPaneId: string): AgentTeamKernelPaneLabelClearing | undefined {
    if (!value || typeof value !== 'object' || Array.isArray(value)) return undefined
    const result = value as Partial<AgentTeamKernelPaneLabelClearing>
    if (result.operation !== 'clearPaneLabel' || result.capability !== 'workerLifecycle') return undefined
    if (result.readOnly !== false || result.stateFilesRead !== false || result.stateFilesWritten !== false || result.tmuxMutation !== true) return undefined
    const paneId = compactTmuxPaneId(result.paneId || requestedPaneId)
    if (result.ok === true) {
      if (!paneId || result.cleared !== true) return undefined
      return {
        ok: true,
        operation: 'clearPaneLabel',
        capability: 'workerLifecycle',
    ...
    if (result.ok !== false || result.cleared !== false) return undefined
    const failureKind = typeof result.failureKind === 'string' && ['unsupported-operation', 'pane-not-found', 'invalid-pane-id', 'invalid-target', 'invalid-session', ...AGENTTEAM_KERNEL_CUTOVER_FAILURE_KINDS].includes(result.failureKind)
      ? result.failureKind as AgentTeamKernelWorkerLifecycleFailureKind
      : 'previous-helper-failure'
    const reason = compactKernelText(`Go worker lifecycle clearPaneLabel unavailable (${failureKind})`, 'Go worker lifecycle clearPaneLabel unavailable (previous-helper-failure)')
    return {
      ok: false,
      operation: 'clearPaneLabel',
      capability: 'workerLifecycle',
  ...
  function validateTeammatePaneCreationResult(value: unknown, requestedTarget: string): AgentTeamKernelTeammatePaneCreation | undefined {
    ...
    if (result.operation !== 'createTeammatePane' || result.capability !== 'workerLifecycle') return undefined
    if (result.readOnly !== false || result.stateFilesRead !== false || result.stateFilesWritten !== false || result.tmuxMutation !== true) return undefined
    const target = compactTmuxWindowTarget(result.target || requestedTarget)
    const paneId = compactTmuxPaneId(result.paneId)
    if (result.ok === true) {
      if (!target || !paneId || result.created !== true) return undefined
      return {
        ok: true,
        operation: 'createTeammatePane',
        capability: 'workerLifecycle',
    ...
    if (result.ok !== false || result.created !== false) return undefined
    const failureKind = typeof result.failureKind === 'string' && ['unsupported-operation', 'pane-not-found', 'invalid-pane-id', 'invalid-target', 'invalid-cwd', 'invalid-start-command', ...AGENTTEAM_KERNEL_CUTOVER_FAILURE_KINDS].includes(result.failureKind)
      ? result.failureKind as AgentTeamKernelWorkerLifecycleFailureKind
      : 'previous-helper-failure'
    const reason = compactKernelText(`Go worker lifecycle createTeammatePane unavailable (${failureKind})`, 'Go worker lifecycle createTeammatePane unavailable (previous-helper-failure)')
    return {
      ok: false,
      operation: 'createTeammatePane',
      capability: 'workerLifecycle',
  ...
  return {
    ...
    async createDetachedSwarmWindowAsync(sessionName, windowName, signal) {
      const requestedSessionName = compactTmuxSessionName(sessionName)
      const requestedWindowName = compactTmuxWindowName(windowName)
      if (!requestedSessionName) return workerLifecycleUnavailableDetachedSwarmWindowCreation(sessionName, windowName, 'invalid-session')
      if (!requestedWindowName) return workerLifecycleUnavailableDetachedSwarmWindowCreation(requestedSessionName, windowName, 'invalid-window-name')
      const helperResult = await callHelperAsync<unknown>('workerLifecycle', { operation: 'createDetachedSwarmWindow', sessionName: requestedSessionName, windowName: requestedWindowName }, signal)
      const parsed = validateDetachedSwarmWindowCreationResult(helperResult, requestedSessionName, requestedWindowName)
      if (parsed) return parsed
      if (helperResult !== undefined) {
        recordRuntimeFallback('helper-incompatible-response', 'workerLifecycle createDetachedSwarmWindow async result shape')
      }
      return workerLifecycleUnavailableDetachedSwarmWindowCreation(requestedSessionName, requestedWindowName, cutoverFailureKind ?? 'previous-helper-failure')
    },
    ...
    async refreshWindowPaneLabelsAsync(target, signal) {
      const requestedTarget = compactTmuxWindowTarget(target)
      if (!requestedTarget) return workerLifecycleUnavailableWindowPaneLabelsRefresh(target, 'invalid-target')
      const helperResult = await callHelperAsync<unknown>('workerLifecycle', { operation: 'refreshWindowPaneLabels', target: requestedTarget }, signal)
      const parsed = validateWindowPaneLabelsRefreshResult(helperResult, requestedTarget)
      if (parsed) return parsed
      if (helperResult !== undefined) {
        recordRuntimeFallback('helper-incompatible-response', 'workerLifecycle refreshWindowPaneLabels async result shape')
      }
      return workerLifecycleUnavailableWindowPaneLabelsRefresh(requestedTarget, cutoverFailureKind ?? 'previous-helper-failure')
    },
    async setPaneLabelAsync(paneId, label, signal) {
      const requestedPaneId = compactTmuxPaneId(paneId)
      if (!requestedPaneId) return workerLifecycleUnavailablePaneLabelSetting(paneId, 'invalid-pane-id')
      if (!isValidPaneLabelArgument(label)) return workerLifecycleUnavailablePaneLabelSetting(requestedPaneId, 'invalid-label')
      const helperResult = await callHelperAsync<unknown>('workerLifecycle', { operation: 'setPaneLabel', paneId: requestedPaneId, label }, signal)
      const parsed = validatePaneLabelSettingResult(helperResult, requestedPaneId)
      if (parsed) return parsed
      if (helperResult !== undefined) {
        recordRuntimeFallback('helper-incompatible-response', 'workerLifecycle setPaneLabel async result shape')
      }
      return workerLifecycleUnavailablePaneLabelSetting(requestedPaneId, cutoverFailureKind ?? 'previous-helper-failure')
    },
    async clearPaneLabelAsync(paneId, signal) {
      const requestedPaneId = compactTmuxPaneId(paneId)
      if (!requestedPaneId) return workerLifecycleUnavailablePaneLabelClearing(paneId, 'invalid-pane-id')
      const helperResult = await callHelperAsync<unknown>('workerLifecycle', { operation: 'clearPaneLabel', paneId: requestedPaneId }, signal)
    ...
    async createTeammatePaneAsync(input, signal) {
      ...
      if (!requestedTarget) return workerLifecycleUnavailableTeammatePaneCreation(input?.target, input?.leaderPaneId, 'invalid-target')
      const requestedLeaderPaneId = compactTmuxPaneId(input?.leaderPaneId)
      if (!requestedLeaderPaneId) return workerLifecycleUnavailableTeammatePaneCreation(requestedTarget, input?.leaderPaneId, 'invalid-pane-id')
      if (typeof input?.hasLeaderLayout !== 'boolean') return workerLifecycleUnavailableTeammatePaneCreation(requestedTarget, requestedLeaderPaneId, 'helper-incompatible-response')
      if (!isValidOptionalTmuxOpaqueArgument(input.cwd)) return workerLifecycleUnavailableTeammatePaneCreation(requestedTarget, requestedLeaderPaneId, 'invalid-cwd')
      if (!isValidOptionalTmuxOpaqueArgument(input.startCommand)) return workerLifecycleUnavailableTeammatePaneCreation(requestedTarget, requestedLeaderPaneId, 'invalid-start-command')
      const params: Record<string, unknown> = {
        operation: 'createTeammatePane',
        target: requestedTarget,
        leaderPaneId: requestedLeaderPaneId,
        hasLeaderLayout: input.hasLeaderLayout,
      }
      if (input.cwd) params.cwd = input.cwd
      if (input.startCommand) params.startCommand = input.startCommand
      const helperResult = await callHelperAsync<unknown>('workerLifecycle', params, signal)
      const parsed = validateTeammatePaneCreationResult(helperResult, requestedTarget)
      if (parsed) return parsed
      if (helperResult !== undefined) {
        recordRuntimeFallback('helper-incompatible-response', 'workerLifecycle createTeammatePane async result shape')
      }
      return workerLifecycleUnavailableTeammatePaneCreation(requestedTarget, requestedLeaderPaneId, cutoverFailureKind ?? 'previous-helper-failure')
    },
