...
The panel intentionally does **not** focus tmux panes, perform task/message CRUD, or mark mailbox items delivered/read. It also does not read full mailbox/report bodies during panel load/render/fingerprint. Use tmux for pane navigation, and use tools for collaboration work. `/team` is for local runtime visibility, recovery, and cleanup. Expanded Details use an internal reader so long descriptions and compact history summaries remain readable without flooding terminal scrollback.
...
Runtime state is stored under `~/.pi/agent/agentteam/` (`teams/<storage-key>/team.json`, `teams/<storage-key>/inboxes/`, `teams/<storage-key>/outbox.json`, `teams/<storage-key>/runtime.json`, `sessions/`, and `worker-sessions/`). `config.json` lives in the same directory. Set `PI_AGENTTEAM_HOME` for testing or temporary sandboxes; `/team config show` displays the effective path and role models.

Team identity uses safe ASCII slugs for new teams. Separator-wrapped safe names are trimmed (`---Shared Team---` and `...Shared Team...` become `shared-team`), while separator-only, punctuation-only, Chinese punctuation-only, and Chinese-only names are rejected with guidance to provide an ASCII name/slug. New team records carry scoped identity metadata (`teamId`, `projectKey`, `displayName`, `slug`, and optional `legacyName` only for legacy/effective identity visibility), allowing the same display/slug to coexist across different projects. Same-project duplicate/collision details include the existing leader cwd, window target, pane id, and session file.

Session bindings are identity-first on disk (`teamId`, `projectKey`, `identityKey`, `teamSlug`) while keeping `teamName`/`memberName` for compatibility. Legacy session bindings that only contain `{ teamName, memberName }` still resolve existing attached teams/members. Legacy no-identity teams, including `teams/-`, are kept safe: AgentTeam does not automatically delete, rename, migrate, recover, or take them over. Repository and `/team` read models can show compact effective identity/`legacyName` markers for those teams without writing back to legacy `team.json` or marking mailbox messages read/delivered.
...
State/read-model profiling is opt-in: `npm run bench:state-read-model` runs `PI_AGENTTEAM_PROFILE=1 node tests/bench/team-read-model-baseline.cjs` and emits explicit JSON for the deterministic `/team` baseline. Panel/tmux refresh profiling is also opt-in: `npm run bench:team-panel-tmux` runs `PI_AGENTTEAM_PROFILE=1 node tests/bench/team-panel-tmux-refresh-v0415.cjs` and emits attached/global warm-refresh dataLoad/render p50/p95, tmux command counts, requestRender/cacheHit/diffChanged metrics, and no-sentinel leak evidence. Normal user/tool output is not polluted when profiling is off, and these benches are baseline/profiling gates only, not p95 release target pass/fail claims.
...
Go kernel work is GitHub-only and optional in this release line. `docs/decisions/0001-replaceable-go-kernel.md` records the accepted planning direction: TypeScript remains the mandatory pi/npm control plane, and Go work must be optional, replaceable, disabled/fallback-safe, and limited to profiling-proven compact helper paths. `docs/go-kernel-port-audit.md` records candidate seams and boundaries; `docs/perf/go-kernel-slice7-checkpoint.md` summarizes the Slice 0-7 benchmark/perf review commands and shadow diagnostics. This is not a native binary commitment, not runtime `/team` UI, and not npm publish guidance.
...
- [ ] Optional Go kernel checkpoint sanity
  - Review `docs/perf/go-kernel-slice7-checkpoint.md` for benchmark-only shadow diagnostics and commands; do not expose those diagnostics in `/team`, add native binaries, or make Go required/default for runtime.
  - Preserve visible tmux panes, leader-gated governance, explicit mailbox/report full-text boundaries, explicit PlanRun progression, no hidden scheduler/autopilot, no worker-spawns-worker, and legacy state compatibility.
...
- Rollback/migration: pin or reinstall a rollback package version (for example npm `pi-agentteam@0.5.0`) and respawn workers. There is no automatic in-process switch from bridge-only delivery to terminal transport, and there is no automatic migration/delete/rename/takeover for legacy team state such as `teams/-`; bridge unavailable appears as public `busy`/`error` worker health with diagnostics. For manual smoke, use a clean `PI_AGENTTEAM_HOME` and respawn old workers so bridge leases exist.
