...
const STEP6_BATCH3_CLUSTERS = Object.freeze({
  ...
  windowLabelMutation: Object.freeze({
    ...
    rationale: 'Step 6 batch 3 deletes replaced non-destructive window/label mutation gate and cutover suites after current guard migration proves exact argv, no hidden TS fallback, compact no-leak diagnostics, and no wake/send-keys/destructive lifecycle expansion.',
  }),
  teammatePaneLifecycleMutation: Object.freeze({
    ...
    expectedSuiteCount: 8,
    recommendedAction: 'delete-replaced',
    rationale: 'Step 6 batches 4 and 5 delete replaced high-risk creation lifecycle and pane-scoped killPane gate/cutover suites after current guard migration proves exact createTeammatePane/new-session/new-window/kill-pane argv, compact throwing or no-throw public behavior as appropriate, and no wake/send-keys/kill-window/kill-session/respawn-pane/buffer expansion.',
  }),
  clearPaneLabelSync: Object.freeze({
    ...
    expectedSuiteCount: 2,
    recommendedAction: 'delete-replaced',
    rationale: 'Step 6 batch 5 deletes replaced sync clear-label gate/cutover suites after current guard migration proves clearPaneLabelSync reuses the existing clearPaneLabel sync adapter path, preserves no-throw void cleanup behavior, avoids a new Go operation/native smoke key, and does not broaden destructive lifecycle authority.',
  }),
})
