const GO_REFRESH_WINDOW_PANE_LABELS_GATE_SCHEMA_VERSION = 1
const GO_REFRESH_WINDOW_PANE_LABELS_GATE_THEME = 'v0.6.73 Go refresh window pane labels gate'
const PACKAGE_VERSION = '0.6.8'
const HELPER_VERSION = '0.3.0-read-model-shadow'
const PROTOCOL_VERSION = 1
const CAPABILITY = 'workerLifecycle'
const FUTURE_OPERATION = 'refreshWindowPaneLabels'
...
const FUTURE_ADAPTER_DELEGATION = 'createAgentTeamKernelAdapter().refreshWindowPaneLabelsAsync(target, signal)'
...
const CURRENT_TYPESCRIPT_COMMAND_SURFACE = Object.freeze([
  Object.freeze({
    ...
    runTmuxNoThrowAsyncCall: "runTmuxNoThrowAsync(['set-option', '-w', '-t', target, 'pane-border-status', 'top'], undefined, signal)",
    command: 'set-option',
    scope: 'window',
  ...
  Object.freeze({
    ...
    runTmuxNoThrowAsyncCall: "runTmuxNoThrowAsync(['set-option', '-w', '-t', target, 'pane-border-format', '#{?@agentteam-name,#{@agentteam-name},#{pane_title}}'], undefined, signal)",
    command: 'set-option',
    scope: 'window',
...
const STILL_FORBIDDEN_MUTATING_SCOPE = Object.freeze([
  ...
  'set-option -w automatic-rename markWindowAsAgentTeam',
  'set-option -w allow-rename markWindowAsAgentTeam',
  'set-option -w @agentteam-window markWindowAsAgentTeam',
  'state repository',
...
const WINDOW_EXISTENCE_AUTHORITY = Object.freeze({
  ...
  futurePolicy: 'keep the TypeScript windowExists(target, signal) guard or move it only inside the same refreshWindowPaneLabels slice if documented and tested',
...
const HELPER_CONNECTION_MODEL = Object.freeze({
  ...
  rationale: 'window pane label refresh is low frequency; long-lived helper remains deferred until a separate state/panel/high-frequency gate',
})
...
const goRefreshWindowPaneLabelsGate = Object.freeze({
  schemaVersion: GO_REFRESH_WINDOW_PANE_LABELS_GATE_SCHEMA_VERSION,
  theme: GO_REFRESH_WINDOW_PANE_LABELS_GATE_THEME,
  ...
  currentGoMutatingTmuxCommands: true,
