...
const REQUIRED_DOC = [
  ...
  'Required capabilities are `health`, `profile`, `tmuxSnapshotParse`, and `compactReadModelFingerprint`, with `businessPathsConnected: false`.',
...
function assertKernelSourceInvariants(root) {
  ...
  assert.equal(/package\.json|node_modules|import\.meta\.url|__dirname|process\.cwd\(\)/i.test(kernel), false, 'kernel must not discover package layout by default')
