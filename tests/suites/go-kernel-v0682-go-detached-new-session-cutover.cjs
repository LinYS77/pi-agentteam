...
function assertRuntimeCutover(root) {
  ...
  assert.equal(windowsSource.includes("import { runTmuxAsync } from './client.js'"), false, `${RUNTIME_FILE} no longer needs direct tmux client import after v0.6.84`)
  ...
  // v0.6.82 fixture/doc remain historical cutover evidence; current source may include the later v0.6.84 authorized detached new-window cutover.
  assert.equal(ensureBody.includes(PRESERVED_TYPESCRIPT_SURFACE.newWindowCall), false, `${RUNTIME_FILE} detached new-window direct TS fallback removed after authorized v0.6.84 cutover`)
  ...
  assert.equal([...ensureBody.matchAll(/runTmuxAsync\(\['new-window'/g)].length, 0, `${RUNTIME_FILE} must not retain direct detached new-window fallback after authorized v0.6.84 cutover`)
...
function assertArtifactPipelineAndNative(root) {
  ...
  assert.equal(typeof manifest.artifact.size, 'number')
  assert.equal(typeof manifest.artifact.sha256, 'string')
  assert.equal(manifest.artifact.sha256, sha256(root, NATIVE_ARTIFACT_SNAPSHOT.helperPath))
