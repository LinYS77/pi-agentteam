...
`), helperPath => {
      ...
      assert.equal(fallbackCalls, 1, 'go mode must preserve migration TypeScript fallback')
      assert.equal(goSnapshot.ok, true)
      assert.equal(go.metadata().kernel.fallbackKind, 'missing-helper')
    })
