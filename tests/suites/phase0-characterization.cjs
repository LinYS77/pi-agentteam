...
module.exports = {
  ...
  async run(env) {
    ...
    assert.equal(modules.runtimeBridge.BRIDGE_PACKAGE_VERSION, '0.5.5', 'bridge package version should match approved v0.5.5 release target; npm v0.5.0 remains documented rollback baseline')

    const packageJson = JSON.parse(fs.readFileSync(path.join(__dirname, '..', '..', 'package.json'), 'utf8'))
    assert.equal(packageJson.version, '0.5.5', 'package version should match approved v0.5.5 release target while v0.5.0 remains the documented rollback baseline')
