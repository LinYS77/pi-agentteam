...
module.exports = {
  ...
  async run(env) {
    ...
    assert.equal(modules.runtimeBridge.BRIDGE_PACKAGE_VERSION, '0.6.4', 'bridge package version should match approved v0.6.4 release target; npm v0.5.0 remains documented rollback baseline')

    const packageJson = JSON.parse(fs.readFileSync(path.join(__dirname, '..', '..', 'package.json'), 'utf8'))
    assert.equal(packageJson.version, '0.6.4', 'package version should match approved v0.6.4 release target while v0.5.0 remains the documented rollback baseline')
