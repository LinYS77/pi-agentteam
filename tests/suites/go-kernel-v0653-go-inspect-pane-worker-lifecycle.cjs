...
function assertContractAndRuntime(env) {
  ...
  assert.deepEqual(contract.AGENTTEAM_KERNEL_FUTURE_WORKER_LIFECYCLE_CONTRACT.activeOperations, [ACTIVE_OPERATION])
  assert.equal(contract.AGENTTEAM_KERNEL_FUTURE_WORKER_LIFECYCLE_CONTRACT.unsupportedOperationsFailClosed, true)
  assert.equal(typeof kernel.createAgentTeamKernelAdapter({ mode: 'typescript', env: {} }).inspectWorkerPane, 'function')
...
function assertSourceAndMetadata(root) {
  ...
  assertIncludes(contract, "AGENTTEAM_KERNEL_FUTURE_WORKER_LIFECYCLE_CONTRACT_STATUS = 'runtime-inspect-pane-only'", CONTRACT)
