...
function assertCurrentTypescriptRuntimeState(root) {
  ...
  assertIncludes(panesSource, CURRENT_TYPESCRIPT_CLEAR_PANE_LABEL_SYNC_SURFACE.importSurface, `${RUNTIME_FILE} runTmuxNoThrow import`)
  ...
  assertIncludes(clearSyncBody, CURRENT_TYPESCRIPT_CLEAR_PANE_LABEL_SYNC_SURFACE.unsetLabelCall, `${RUNTIME_FILE} clearPaneLabelSync unset call`)
  assertIncludes(clearSyncBody, CURRENT_TYPESCRIPT_CLEAR_PANE_LABEL_SYNC_SURFACE.clearTitleCall, `${RUNTIME_FILE} clearPaneLabelSync clear-title call`)
  assert.equal([...clearSyncBody.matchAll(/runTmuxNoThrow\(/g)].length, 2, `${RUNTIME_FILE} clearPaneLabelSync keeps exactly two no-throw calls`)
  assert.equal(clearSyncBody.includes('createAgentTeamKernelAdapter'), false, `${RUNTIME_FILE} clearPaneLabelSync must not cut over in this gate`)
