...
module.exports = {
  ...
  async run(env) {
    ...
    const planOneOutbox = modules.state.getOutboxEffect('full-suite-team', res.details.outboxEffectId)
    ...
    assert.equal(planOneOutbox?.idempotencyKey, ['spawn-initial-worker-delivery', 'full-suite-team', 'plan-one'].join(':'), 'spawn bootPrompt outbox idempotency key should preserve current team/member shape')
    ...
    assert.equal(bridgeTimeoutOutbox?.idempotencyKey, ['spawn-initial-worker-delivery', 'full-suite-team', 'bridge-timeout-worker'].join(':'), 'bridge timeout spawn should preserve initial delivery idempotency key shape')
    ...
    assert.equal(bridgeReadyOutbox?.idempotencyKey, ['spawn-initial-worker-delivery', 'full-suite-team', 'bridge-ready-worker'].join(':'), 'bridge-ready spawn should preserve initial delivery idempotency key shape')
