...
module.exports = {
  ...
  async run(env) {
    ...
    assert.equal(defaultMetadata.kernel.requestedMode, 'disabled')
    assert.equal(defaultMetadata.kernel.requestedKnownKernel, true)
    assert.equal(defaultMetadata.kernel.mode, 'typescript')
    assert.equal(defaultMetadata.kernel.enabled, false)
...
`), helperPath => {
    ...
    assert.equal(goFallbackCalls, 1, 'go missing helper must keep migration TS fallback behavior')
    assert.equal(goMissingSnapshot.ok, true)
    assert.equal(goMissingSnapshot.panes[0].paneId, '%ts')
    ...
    assert.equal(goMissing.metadata().kernel.fallbacks, 1)
    assert.equal(goMissing.metadata().kernel.fallbackKind, 'missing-helper')
    assertNoCutoverDiagnostic(goMissing.metadata(), 'go missing helper')
