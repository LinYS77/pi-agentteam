...
const REQUIRED_DOC = [
  ...
  "Pane setup `runTmuxAsync(['list-panes', '-t', initialTarget, '-F', '#{pane_id}'], undefined, signal)` remains TypeScript-owned.",
...
const REQUIRED_ROADMAP = [
  ...
  'pane setup list-panes/post-creation list-windows/new-session/new-window/marking/labels remain TypeScript-owned',
...
function assertFacadeSource(root) {
  ...
  assertIncludes(ensureBody, 'const panes = (await runTmuxAsync([\'list-panes\', \'-t\', initialTarget, \'-F\', \'#{pane_id}\']', 'pane setup list-panes remains TS-owned')
...
async function withPatchedDetachedDeps(env, patch, callback) {
  ...
  const fakeClient = {
    ...
    async execAsync(args) {
      tmuxCalls.push(args)
      if (args[0] === 'list-panes' && args[1] === '-t' && args[3] === '-F' && args[4] === '#{pane_id}') return '%leader\n%second'
...
async function assertDetachedRuntime(env) {
  ...
  await withPatchedDetachedDeps(env, {
    ...
    assert.deepEqual(tmuxCalls, [['list-panes', '-t', 'pi-agentteam:@7', '-F', '#{pane_id}']], 'detached branch should only use pane setup list-panes when discovery succeeds')
  ...
  await withPatchedDetachedDeps(env, {
    ...
    assert.deepEqual(tmuxCalls, [['list-panes', '-t', 'pi-agentteam:@7', '-F', '#{pane_id}']], 'missing binding should not use hidden target-based display-message fallback')
  })
}
