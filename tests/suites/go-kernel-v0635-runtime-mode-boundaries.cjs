...
function assertNoBroadGoControlPlane(root) {
  ...
  assert.match(goSource, /exec\.CommandContext\(ctx, "tmux", "list-panes", "-a", "-F", tmuxPaneSnapshotFormat\)/, 'post-v0.6.49 Go helper may execute only narrow tmux snapshot capture')
  assert.equal(/send-keys|split-window|new-window|kill-pane|display-message|exec\.Command\s*\(/i.test(goSource), false, 'Go helper must not execute broad tmux lifecycle commands or shells')
