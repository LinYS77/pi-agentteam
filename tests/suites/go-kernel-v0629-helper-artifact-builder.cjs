...
function assertPositiveArtifact(root, outputRoot, result, recordPath) {
  ...
  assert.equal(result.summary.smoke.workerLifecycleKillPane, true)
  assert.equal(result.summary.smoke.workerLifecycleCreateTeammatePane, true)
  assert.equal(result.summary.smoke.workerLifecycleCreateDetachedSwarmSession, true)
  assert.equal(result.summary.smoke.workerLifecycleCreateDetachedSwarmWindow, true)
  ...
  assert.deepEqual(provenance.smoke.workerLifecycleKillPane.acceptedFailureKinds, ['invalid-pane-id'])
  ...
  assert.deepEqual(provenance.smoke.workerLifecycleCreateDetachedSwarmWindow.acceptedFailureKinds, ['invalid-session'])
...
function runCliFakePositive(root) {
  ...
  try {
    ...
    assert.equal(summary.smoke.workerLifecycleKillPane, true)
    assert.equal(summary.smoke.workerLifecycleCreateTeammatePane, true)
    assert.equal(summary.smoke.workerLifecycleCreateDetachedSwarmSession, true)
    assert.equal(summary.smoke.workerLifecycleCreateDetachedSwarmWindow, true)
