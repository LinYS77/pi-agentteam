...
module.exports = {
  ...
  async run(env) {
    ...
    assert.equal(pkg.version, '0.6.8', 'release package version should match approved v0.6.8 target')
    assert.equal(pkg.scripts?.test, 'node tests/run.cjs --tier default')
    assert.equal(pkg.scripts?.['test:smoke'], 'node tests/run.cjs --tier smoke')
    assert.equal(pkg.scripts?.['test:core'], 'node tests/run.cjs --tier core')
    assert.equal(pkg.scripts?.['test:go-current'], 'node tests/run.cjs --tier go-current')
    assert.equal(pkg.scripts?.['test:audit'], 'node tests/run.cjs --tier audit')
    assert.equal(pkg.scripts?.['test:regression'], 'node tests/run.cjs --tier regression')
    assert.equal(pkg.scripts?.['test:list'], 'node tests/run.cjs --list')
    ...
    assert.ok(pkg.scripts?.check?.includes('npm run test:regression'), 'check should run the full test regression suite')
    assert.equal(pkg.scripts?.check?.includes('npm test'), false, 'check must not rely on reduced default npm test')
