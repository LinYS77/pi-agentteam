...
module.exports = {
  ...
  async run(env) {
    ...
    for (const file of publicDeclarationFiles) {
      const declaration = fs.readFileSync(path.join(declarationDir, file), 'utf8')
      for (const token of forbiddenPublicSurfaceTokens) {
        assert.equal(declaration.includes(token), false, `${file} public declaration should not expose internal token ${token}`)
      }
      assert.equal(declaration.includes('internalTypes'), false, `${file} public declaration should not reference internalTypes`)
      assert.equal(declaration.includes('../app/'), false, `${file} public declaration should not alias app internals`)
    }
    const publicTypesDeclaration = fs.readFileSync(path.join(declarationDir, 'types.d.ts'), 'utf8')
