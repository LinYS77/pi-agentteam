...
const EXPECTED_TIER_COUNTS_POST_T039 = Object.freeze({
  default: 85,
  smoke: 10,
  core: 59,
  'go-current': 26,
...
const STEP6_DELETED_READ_ONLY_FACADE_SUITE_FILES = Object.freeze([
...
const STEP6_OUT_OF_SCOPE_READ_ONLY_FACADE_SUITE_FILES = Object.freeze([
...
module.exports = {
  ...
  async run(env) {
    ...
    assert.equal(allSuites.length, EXPECTED_TIER_COUNTS_POST_T039.regression, 'manifest should encode the post-T039 discovered suite count')
    assert.deepEqual(summarizeSelection(allSuites), EXPECTED_TIER_COUNTS_POST_T039, 'suite tier summary should encode the post-T039 topology')
    for (const [tier, suites] of Object.entries(tierSelections)) {
      assert.equal(suites.length, EXPECTED_TIER_COUNTS_POST_T039[tier], `${tier} tier count should match post-T039 topology`)
    }
    ...
    for (const file of STEP6_DELETED_READ_ONLY_FACADE_SUITE_FILES) {
      assert.equal(allSuites.includes(file), false, `${file} should be absent after Step 6 read-only facade/orchestration deletion`)
      assert.equal(auditSuites.includes(file), false, `${file} should not remain in audit after Step 6 read-only facade/orchestration deletion`)
      assert.equal(regressionSuites.includes(file), false, `${file} should not remain in regression after Step 6 read-only facade/orchestration deletion`)
    }
    for (const file of STEP6_OUT_OF_SCOPE_READ_ONLY_FACADE_SUITE_FILES) {
      assert.ok(allSuites.includes(file), `${file} should remain discoverable because it is out of T039 scope`)
      assert.ok(auditSuites.includes(file), `${file} should remain audit coverage because it is out of T039 scope`)
      assert.ok(regressionSuites.includes(file), `${file} should remain regression coverage because it is out of T039 scope`)
    }
