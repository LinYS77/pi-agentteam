...
function assertNoCiReleaseOrPackageScripts(root) {
  ...
  for (const name of fs.readdirSync(workflows).filter(value => /\.(?:ya?ml)$/i.test(value))) {
    ...
    assert.equal(/actions\/upload-artifact|gh\s+release|npm\s+publish|go\s+build/i.test(source), false, `${name} must not add artifact/release/build workflow behavior in Slice 4`)
  }
}
