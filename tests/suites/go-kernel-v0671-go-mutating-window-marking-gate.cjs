...
function assertAuthorizedRuntimeCutover(root) {
  ...
  for (const command of FORBIDDEN_CURRENT_GO_TMUX_COMMANDS.filter(command => !['set-option', 'select-pane', 'split-window', 'select-layout', 'resize-pane', 'new-session', 'new-window', 'kill-pane'].includes(command))) assert.equal(goSource.includes(`"${command}"`), false, `${GO_SOURCE} must not add forbidden tmux command ${command}`)
  assertIncludes(goSource, 'exec.CommandContext(ctx, "tmux", "kill-pane", "-t", paneID)', `${GO_SOURCE} later v0.6.86 authorized exact argv killPane`)
