...
function assertAuthorizedRuntimeCutover(root) {
  ...
  for (const command of FORBIDDEN_CURRENT_GO_TMUX_COMMANDS.filter(command => !['set-option', 'select-pane', 'split-window', 'select-layout', 'resize-pane', 'new-session'].includes(command))) assert.equal(goSource.includes(`"${command}"`), false, `${GO_SOURCE} must not add forbidden tmux command ${command}`)
