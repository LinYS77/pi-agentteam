...
module.exports = {
  ...
  async run(env) {
    ...
    await withTempHome(env.modules, 'tiny', async () => {
      ...
      assert.equal(result.attached.requestRenderCount, 0, 'tiny attached unchanged refresh should not request render')
      assert.equal(result.global.requestRenderCount, 0, 'tiny global unchanged refresh should not request render')
      assert.equal(result.attached.diffChangedCount, 0, 'tiny attached unchanged refresh should not record diffChanged')
      assert.equal(result.global.diffChangedCount, 0, 'tiny global unchanged refresh should not record diffChanged')
