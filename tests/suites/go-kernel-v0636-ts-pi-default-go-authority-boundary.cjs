...
function assertGoAuthorityBounded(root) {
  ...
  assertIncludes(goSource, 'var capabilities = []string{"health", "profile", "tmuxSnapshotParse", "tmuxSnapshotCapture", "compactReadModelFingerprint", "workerLifecycle", "tmuxAvailability"}', 'kernel/go/agentteam-kernel/main.go')
  ...
  assertIncludes(goSource, 'case "tmuxAvailability":', 'kernel/go/agentteam-kernel/main.go')
  assertIncludes(goSource, 'case "inspectPane":', 'kernel/go/agentteam-kernel/main.go')
  assertIncludes(goSource, 'case "listAgentTeamPanes":', 'kernel/go/agentteam-kernel/main.go')
  ...
  assert.match(goSource, /"scope":\s+"skeleton-only",/, 'kernel/go/agentteam-kernel/main.go should keep skeleton profile scope')
  assert.match(goSource, /"stateConnected":\s+false,/, 'kernel/go/agentteam-kernel/main.go should keep state disconnected')
  assert.match(goSource, /"tmuxConnected":\s+false,/, 'kernel/go/agentteam-kernel/main.go should keep broad tmux disconnected')
  assert.match(goSource, /"panelConnected":\s+false,/, 'kernel/go/agentteam-kernel/main.go should keep panel disconnected')
  assert.match(goSource, /"taskReportPlanRunConnected":\s+false,/, 'kernel/go/agentteam-kernel/main.go should keep task/report/PlanRun disconnected')
  ...
  assert.match(goSource, /"workerLifecycleListAgentTeamPanesConnected":\s+true,/, 'kernel/go/agentteam-kernel/main.go should expose listAgentTeamPanes profile connectivity')
  assert.match(goSource, /"tmuxAvailabilityConnected":\s+true,/, 'kernel/go/agentteam-kernel/main.go should expose tmuxAvailability profile connectivity')
