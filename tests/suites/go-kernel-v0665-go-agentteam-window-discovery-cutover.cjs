...
const REQUIRED_DOC = [
  ...
  '`has-session`, `new-session`, `new-window`, list-panes during pane setup, display-message current target fallbacks, marking, labels, kill, state/task/UI/release/package remain TypeScript-owned.',
...
const REQUIRED_ROADMAP = [
  ...
  'has-session/new-session/new-window/marking/labels/pane setup remain TypeScript-owned',
  '**v0.6.65 Go agentteam window discovery cutover**',
]
...
function assertFixtureShape(root) {
  ...
  assert.equal(goAgentTeamWindowDiscoveryCutover.hasSessionMigrated, false)
...
function assertFacadeSource(root) {
  ...
  assertIncludes(ensureBody, "runTmuxNoThrowAsync(['has-session', '-t', SWARM_SESSION]", 'has-session remains TS-owned')
