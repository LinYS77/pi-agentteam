...
module.exports = {
  ...
  async run(env) {
    ...
    assert.deepEqual(defaultMetadata.kernel.capabilities, ['health', 'profile', 'tmuxSnapshotParse', 'compactReadModelFingerprint'])
    ...
    assert.deepEqual(defaultHealth.capabilities, ['health', 'profile', 'tmuxSnapshotParse', 'compactReadModelFingerprint'])
