...
function assertRuntimeAndGoUnchanged(root) {
  ...
  for (const command of BROAD_GO_LIFECYCLE_COMMANDS.filter(command => !['select-pane', 'split-window', 'new-session', 'new-window', 'kill-pane'].includes(command))) assert.equal(goSource.includes(command), false, `${GO_SOURCE} must not add broad tmux lifecycle command ${command}`)
  assertIncludes(goSource, 'exec.CommandContext(ctx, "tmux", "kill-pane", "-t", paneID)', `${GO_SOURCE} later v0.6.86 permits only exact argv killPane`)
  assert.equal([...goSource.matchAll(/exec\.CommandContext\(ctx, "tmux", "kill-pane", "-t", paneID\)/g)].length, 1, `${GO_SOURCE} must contain exactly one authorized killPane command`)
