...
function assertRuntimeAndGoUnchanged(root) {
  ...
  for (const command of BROAD_GO_LIFECYCLE_COMMANDS.filter(command => !['select-pane', 'split-window', 'new-session', 'new-window'].includes(command))) assert.equal(goSource.includes(command), false, `${GO_SOURCE} must not add broad tmux lifecycle command ${command}`)
