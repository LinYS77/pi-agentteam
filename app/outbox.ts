...
import type { TeamMessagePriority, TeamMessageType, TeamMessageWakeHint } from '../internalTypes.js'

export const OUTBOX_EFFECT_KINDS = Object.freeze([
...
export const OUTBOX_EFFECT_STATUSES = Object.freeze(['pending', 'done', 'failed'] as const)
...
export function isOutboxEffectKind(value: unknown): value is OutboxEffectKind {
  return typeof value === 'string' && (OUTBOX_EFFECT_KINDS as readonly string[]).includes(value)
}

export function isOutboxEffectStatus(value: unknown): value is OutboxEffectStatus {
  return typeof value === 'string' && (OUTBOX_EFFECT_STATUSES as readonly string[]).includes(value)
}
