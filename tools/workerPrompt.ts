...
export function buildWorkerSystemPrompt(input: {
  ...
  return [
    ...
    '- If awakened by an agentteam signal and unread messages may exist, call agentteam_receive when you need full mailbox details; compact wake/projection prompts are reminders, not the full message body.',
    '- Keep progress in task notes; notes are task-local memory and do not notify team-lead. Use agentteam_send for communication and report_done/report_blocked for owner-to-leader action requests.',
    ...
    '- Worker delivery prompts may merge same-task assigned task facts with task-bound mailbox messages; still treat the mailbox as source of truth and call agentteam_receive for full unread details when needed.',
