...
export function executeReceiveMessages(
  ...
  if (!team) {
    return { content: [{ type: 'text' as const, text: 'No current team context.' }], details: {} }
  }
  const recipient = deps.currentActor(ctx)
  if (!team.members[recipient]) {
    return {
      content: [{ type: 'text' as const, text: `Current actor ${recipient} is not a member of team ${team.name}.` }],
