...
1. **v0.6.96 v0.7 release decision package/no-action preflight**：docs/tests/user-decision-preflight for `v0.7.0 = core refactor + performance baseline + bug burn-down release`；status `release-decision-package-ready-for-user-review-no-release-action`, release decision `pending-explicit-user-choice`, `releaseReadyClaim=false`, `releaseActionAuthorized=false`, `releaseActionPerformed=false`, `packageVersion=0.6.8`, release decision package ready for user review; no release action authorized or performed, and final release/tag/npm action still requires explicit user authorization；accepted evidence through v0.6.95 and commit `bab6937 Add v0.7 evidence reconciliation` is summarized for user choice；user options are explicit release mechanics authorization later, additional true interactive/manual operator pass first, or deferral/continued development, with no option selected/executed now；true interactive pi/TUI/operator/model pass remains unperformed and deferred/manual unless release ownership promotes it；continuous no-raw-artifact policy remains in force；`package.json` remains `0.6.8`; worker delivery remains bridge-only and Go `send-keys` / active `wakePane` remain unauthorized；current embedded native helper path remains `native/tmuxSnapshotParse/0.3.0-read-model-shadow/linux-x64-glibc/`, with no native helper path/binary rename, no second platform claim, no default resolver/default Go expansion beyond accepted explicit cutovers, no hidden TypeScript tmux fallback reintroduction, no runtime/source/Go/native/tmux/package/state/task/PlanRun/mailbox/governance/team panel/UI/release-control-plane changes, and no tag/release/npm/signing/tarball/raw-artifact/lockfile/go.mod/go.sum changes，证据见 `docs/perf/v0.6.96-v07-release-decision-package.md`。
1. **v0.6.95 v0.7 evidence reconciliation**：docs/tests/readiness reconciliation for `v0.7.0 = core refactor + performance baseline + bug burn-down release`；status `evidence-reconciled-release-decision-pending-no-release-action`, reconciliation gate `reconciled-current-evidence-chain-no-release-action`, release decision `release-decision-pending-explicit-user-authorization`, `releaseReadyClaim=false`, `releaseActionPerformed=false`, final release/tag/npm action still requires explicit user authorization, `package.json` remains `0.6.8`, and no release/package action is authorized or performed；v0.6.90 readiness inventory, v0.6.91 p95, v0.6.92 deterministic manual RC/operator-seam, v0.6.93 bug ledger, v0.6.94 governance review, and T046 fix now reconcile with current repo behavior；continued no raw artifact policy remains in force；true interactive pi/TUI/operator/model pass remains unperformed and deferred/manual unless release ownership promotes it；worker delivery remains bridge-only and Go `send-keys` / active `wakePane` remain unauthorized；current embedded native helper path remains `native/tmuxSnapshotParse/0.3.0-read-model-shadow/linux-x64-glibc/`, with no native helper path/binary rename, no second platform claim, no default resolver/default Go expansion beyond accepted explicit cutovers, no hidden TypeScript tmux fallback reintroduction, and no state/task/PlanRun/mailbox/governance, team panel/UI, release/package control-plane, runtime/source/Go/native/tmux/package, lockfile/go.mod/go.sum, tag/release/npm, signing/tarball/raw-artifact changes，证据见 `docs/perf/v0.6.95-v07-evidence-reconciliation.md`。
1. **v0.6.94 v0.7 release checklist/governance review**：docs/tests/readiness governance for `v0.7.0 = core refactor + performance baseline + bug burn-down release`；status `release-governance-reviewed-not-release-ready`, governance gate `reviewed-no-release-action-authorized`, `package.json` stays `0.6.8`, and no release/package action is authorized or performed: no `npm version` / `npm publish` / tags / GitHub releases/assets, no release asset upload, no signing/cosign/SLSA/security attestation, no package source approval, no lifecycle hook/optional native dependency/postinstall/download/runtime build flow, and no release/package control-plane migration；current embedded helper path remains `native/tmuxSnapshotParse/0.3.0-read-model-shadow/linux-x64-glibc/`, with no native helper path/binary rename, no second platform claim, no default resolver/default Go expansion beyond accepted explicit cutovers, and no hidden TypeScript tmux fallback reintroduction；true interactive pi/TUI/operator/model pass remains a deferred/manual operator procedure and not a current blocker for this v0.7 readiness track unless release ownership promotes it later；v0.6.91 p95, v0.6.92 operator-seam RC, v0.6.93 bug ledger, and T046 fix are accepted；evidence reconciliation remains outstanding and continued no raw artifact policy remains in force；worker delivery remains bridge-only and Go `send-keys` / active `wakePane` remain unauthorized；mutating lifecycle/state/task/PlanRun/mailbox/governance/team panel/UI/release control-plane migrations, Go/native/tmux/runtime/source changes, lockfiles, package-control, tarballs/signatures/raw artifacts, and release mechanics remain blocked，证据见 `docs/perf/v0.6.94-v07-release-governance-review.md`。
1. **v0.6.93 v0.7 bug burn-down ledger refresh**：docs/tests/readiness ledger for `v0.7.0 = core refactor + performance baseline + bug burn-down release`；status `bug-burndown-ledger-refreshed-not-release-ready`, bug ledger gate `refreshed-no-known-active-test-visible-p0-p1-blockers`, `package.json` remains `0.6.8`; T046 `tools-state.cjs:577` pane-health mismatch is fixed by `e423179 Isolate tools-state tmux snapshot fixture` with evidence in `tests/suites/tools-state.cjs`, and current validation records `node tests/run.cjs tools-state` pass, `npm test` pass, typecheck/boundaries pass, and no known active test-visible P0/P1 blockers after current validation without claiming no bugs in every environment；v0.6.91 clean-temp p95 refresh is accepted and v0.6.92 clean-temp manual RC/operator-seam refresh is accepted；true interactive pi/TUI/operator/model pass is a coverage limitation/deferred operator procedure rather than an active bug unless release governance promotes it；release checklist/governance review and evidence reconciliation remain outstanding, with continued no raw artifact policy in force；worker delivery remains bridge-only and Go `send-keys` / active `wakePane` remain unauthorized；state/task/PlanRun/mailbox/governance, team panel/UI, release/package control-plane migrations, native helper path rename, Go/native/tmux/runtime/source changes, npm/tag/release/assets, lockfiles, lifecycle hooks, package-control, and hidden fallbacks remain deferred/blocked outside this ledger，证据见 `docs/perf/v0.6.93-v07-bug-burndown-ledger.md`。
1. **v0.6.92 clean-temp manual RC/operator-seam evidence refresh**：docs/tests/evidence-only refresh for `v0.7.0 = core refactor + performance baseline + bug burn-down release`；status `manual-rc-operator-seam-refreshed-not-release-ready`, manual RC/operator-seam gate `operator-seam-refreshed-pass`, `package.json` remains `0.6.8`, deterministic temp-home RC harness `passed` with output SHA256 `db96d1153fcae7e46cfb38511a34f9ab9cdbd23bfbbd655d4c2a6f85a264c51f`; coverage is stubbed pi/tmux operator seams, not a true interactive pi/TUI/operator/model pass: 4 `/team config` command seams, 21 `agentteam_*` tool seams, 8 checks passed, clean temp home removed, no `--keep-home` / `--allow-non-empty-home`, and no raw harness JSON/logs/timing/smoke output/terminal logs/screenshots/mailbox/report bodies/transcripts/state archives/secrets/hosted records/tarballs/temp homes checked in；true interactive pi/TUI/operator/model pass was not performed；v0.6.91 clean-temp p95 refresh is accepted and T046 tools-state blocker fix is accepted；bug burn-down ledger, release checklist/governance review, and evidence reconciliation remain outstanding；worker delivery remains bridge-only and Go `send-keys` / active `wakePane` remain unauthorized；state/task/PlanRun/mailbox/governance, team panel/UI, release/package control-plane runtime, Go/native/tmux/runtime/source, npm/tag/release/assets, lockfiles, lifecycle hooks, package-control, hidden fallbacks, and native helper/path changes remain blocked，证据见 `docs/perf/v0.6.92-v07-clean-temp-manual-rc-refresh.md`。
1. **v0.6.91 clean-temp v0.7 p95 evidence refresh**：docs/tests/evidence-only refresh for `v0.7.0 = core refactor + performance baseline + bug burn-down release`；status `p95-refreshed-not-release-ready`, p95 gate `refreshed-pass`, `package.json` remains `0.6.8`, all selected clean-temp `PI_AGENTTEAM_HOME` harnesses passed without `--keep-home` / `--allow-non-empty-home`: task-message-report normal p95 `29.725ms <= 50ms`, large-mailbox p95 `102.84ms <= 150ms`, fsStore lock-wait p95 `10.63ms <= 25ms`, data-change render debounce `2.985 renders/sec <= 4`, spawn bookkeeping p95 `9.098ms <= 100ms`; output JSON stayed under `/tmp` with SHA256-only summary captured and no raw harness JSON/timing records/logs/screenshots/mailbox/report bodies/transcripts/state archives/secrets/hosted records/tarballs/temp homes checked in；manual RC/operator evidence, bug burn-down ledger, release checklist/governance review, and evidence reconciliation remain outstanding；worker delivery remains bridge-only and Go `send-keys` / active `wakePane` remain unauthorized；state/task/PlanRun/mailbox/governance, team panel/UI, release/package control-plane runtime, Go/native/tmux/runtime/source, npm/tag/release/assets, lockfiles, lifecycle hooks, package-control, and native helper/path changes remain blocked，证据见 `docs/perf/v0.6.91-v07-clean-temp-p95-refresh.md`。
1. **v0.6.90 v0.7 readiness burn-down refresh**：gate-only docs/tests inventory for `v0.7.0 = core refactor + performance baseline + bug burn-down release`；accepted head includes v0.6.88 `clearPaneLabelSync` cutover and v0.6.89 bridge-only worker delivery boundary gate, but current status remains `not-release-ready` / `not-release-ready-readiness-inventory-only` until clean-temp `PI_AGENTTEAM_HOME` p95 evidence refresh, clean-temp manual RC/operator evidence, bug burn-down ledger, release checklist/governance review, evidence reconciliation, and no raw artifact check-ins are completed；worker delivery remains bridge-only TypeScript-owned outbox/bridge orchestration and Go `send-keys` / active `wakePane` remain unauthorized；state/task/PlanRun/mailbox/governance, team panel/UI, release/package control-plane runtime, terminal wake/send-keys, native/package/default resolver, npm/tag/release/assets, lockfiles, lifecycle hooks, and runtime/source behavior changes remain blocked without future explicit gates，证据见 `docs/perf/v0.6.90-v07-readiness-burndown-refresh.md`。
