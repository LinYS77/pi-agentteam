...
- `paneExists()` is cut over separately by v0.6.57; `resolvePaneBinding()`, `targetForPaneId()`, `captureCurrentPaneBinding()`, and `resolvePaneBindingAsync()` are later cut over by v0.6.58-v0.6.61; window helpers remain TypeScript-owned.
