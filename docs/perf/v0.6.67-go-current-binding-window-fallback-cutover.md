...
- the detached target-based leader pane `display-message -p -t leaderPaneId '#{window_id}'` fallback is superseded by v0.6.68, while post-creation `list-windows`, pane setup `list-panes`, session/window creation, marking, and label refresh remain TypeScript-owned.
