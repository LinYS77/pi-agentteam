...
- the detached target-based leader pane `display-message -p -t leaderPaneId '#{window_id}'` fallback is superseded by v0.6.68, pane setup `list-panes` is superseded by v0.6.69, and post-creation `list-windows` is superseded by v0.6.70, while session/window creation, marking, and label refresh remain TypeScript-owned.
