...
- the detached target-based leader pane `display-message -p -t leaderPaneId '#{window_id}'` fallback is superseded by v0.6.68, and pane setup `list-panes` is superseded by v0.6.69, while post-creation `list-windows`, session/window creation, marking, and label refresh remain TypeScript-owned.
