...
- Go helper authority remains bounded to JSON-RPC helper responses behind the TS kernel adapter; cutover authority remains `tmuxSnapshotParse` plus v0.6.53 read-only `workerLifecycle.inspectPane`.
- Go cannot create pi commands, tools, renderers, hooks, package/release operations, hosted workflow actions, mutating tmux execution ownership, mutating worker lifecycle ownership, task/report/PlanRun governance, mailbox/state/report full-text authority, or model-callable native/default/release tools.
