...
- `ensureSwarmWindow()` still owns `new-session`, `new-window`, post-creation window lookup, pane setup list-panes, detached target-based leader-pane window lookup, marking, and label refresh; the former `has-session` check is superseded by v0.6.66, and the inside-tmux current binding `display-message` fallback is superseded by v0.6.67.
