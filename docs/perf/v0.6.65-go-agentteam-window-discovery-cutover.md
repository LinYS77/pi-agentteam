...
- `ensureSwarmWindow()` still owns `new-session`, `new-window`, marking, and label refresh; the former `has-session` check is superseded by v0.6.66, the inside-tmux current binding `display-message` fallback is superseded by v0.6.67, detached target-based leader-pane lookup is superseded by v0.6.68, pane setup `list-panes` is superseded by v0.6.69, and post-creation window lookup is superseded by v0.6.70.
