...
- `ensureSwarmWindow()` still owns `new-session`, `new-window`, post-creation window lookup, marking, and label refresh; the former `has-session` check is superseded by v0.6.66, the inside-tmux current binding `display-message` fallback is superseded by v0.6.67, detached target-based leader-pane lookup is superseded by v0.6.68, and pane setup `list-panes` is superseded by v0.6.69.
