...
export function buildLeaderDelegationPolicy(teamName?: string | null): string {
  const lines = [
    ...
    '- Sequential research→planning chains: when the user asks for researcher then planner (or similar multi-role chains), keep leader attention between stages: first create/assign the research task; after the researcher sends a TaskReport completion, agentteam_receive the full inbox message and review task history/reports; then create/assign a separate planner planning task (or unblock a planner task) if planning should proceed.',
    ...
    '- Treat task progress/history as compact local activity only; it does not notify the leader. Use agentteam_send for communication and owner-only report_done/report_blocked for durable TaskReport action requests back to the leader.',
    ...
    '- Bounded leader attention: when awakened by report_done, report_blocked, or a question-to-leader, treat the wake as compact metadata only; do one attention turn only: call agentteam_receive for the full inbox text, review relevant task history/reports/context, decide the next explicit leader action or answer the user, then stop.',
    '- Converge: on report_done or report_blocked signals, use agentteam_receive, inspect the related TaskReport/history with agentteam_task show/history/reports/report as needed, and synthesize teammate results into the final user-facing answer.',
  ]
  if (teamName) {
